...
def test_ambient_file_operation_receives_app_and_context(tmp_path: Path) -> None:
    ...
    bind_run(make_run_log(
        tmp_path, path=str(log), app="rey_analyzer", run_id="r", run_timestamp="ts",
        pipeline="trade_analyzer_generate_apply_ddl",
    ))
    try:
...
def test_ambient_file_operation_omits_absent_identity(tmp_path: Path) -> None:
    # No app identity on the run log -> app is simply absent, exactly as before.
    log = tmp_path / "app.ts.jsonl"
    bind_run(make_run_log(tmp_path, path=str(log), app="", run_id="r",
                          run_timestamp="ts"))
    try:
