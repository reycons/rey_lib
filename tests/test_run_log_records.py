...
def _ctx(tmp_path: Path) -> SimpleNamespace:
    """A context whose log directory is tmp_path, identified as a launch would.

    Run logging consumes an identity; it never mints one. A launch establishes
    it through ``rey_lib.run`` before logging opens, so the fixture does the
    same rather than relying on the write path to invent one.
    """
    ctx = SimpleNamespace(
...
def test_workflow_runner_emits_run_log_records(tmp_path: Path) -> None:
    ...
    # Finalization is deterministic end to end: the summary, then the package
    # it hands off. It no longer attempts interpretation, so a run with no AI
    # records no analysis failure -- there is no analysis to fail.
...
def test_workflow_completion_appends_artifact_manifest(tmp_path: Path) -> None:
    ...
    records = _read(Path(run_log.path()))
    # Finalization appends the package after the summary and stops there.
