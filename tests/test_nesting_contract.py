...
The contract under test:
    ...
    run_log.set_nest_level()   parent_level = level; minimum_nest_level = level + 1;
...
def test_set_establishes_parent_level_and_relative_nesting_starts_below_it(run_log) -> None:
    ...
    ctx = Namespace({})
    assert run_log.set_nest_level("pipeline") == 1      # parent_level = 1
    assert run_log.nest_level() == 1                  # records may be written at 1
    assert run_log.enter() == 2                 # minimum_nest_level = 2
    assert run_log.enter() == 3                 # descends further
...
def test_previous_returns_upward_but_never_below_the_relative_floor(run_log) -> None:
    ...
    ctx = Namespace({})
    run_log.set_nest_level("pipeline")                  # parent 1, minimum 2
    run_log.enter()                             # 2
    run_log.enter()                             # 3
    assert run_log.exit() == 2
...
def test_relative_floor_is_relative_to_the_established_base(run_log) -> None:
    ...
    ctx = Namespace({})
    run_log.set_nest_level("pipeline_step")             # parent 2, minimum 3
    assert run_log.enter() == 3
    assert run_log.exit() == 3             # cannot return to the base itself
    run_log.set_nest_level("app")                       # parent 3, minimum 4
    assert run_log.enter() == 4
...
def test_subsequent_set_resets_the_relative_context_and_rebases(run_log) -> None:
    ...
    run_log.set_nest_level("pipeline")                  # parent 1, minimum 2
    run_log.enter()                             # 2
    run_log.enter()                             # 3 — left deep
    # The next base is authoritative regardless of the abandoned relative context.
    assert run_log.set_nest_level("pipeline_step") == 2  # parent 2, minimum 3
    assert run_log.nest_level() == 2
    assert run_log.enter() == 3
    assert run_log.exit() == 3             # the new floor, not the old one
...
def test_set_next_enters_collection_and_sibling_reopens_peer_branches(tmp_path: Path) -> None:
    ...
    assert run_log.set_nest_level("next") == 4
    assert run_log.set_nest_level("sibling") == 4
