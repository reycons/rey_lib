...
# The weight budget.
#
# A row count bounds the wrong thing on its own. Measured on a file of 5 MB
# rows, a fifty-row page weighed 243 MB -- which is the payload that took the
# Console down, and which LIMIT 50 considers perfectly reasonable.
# ---------------------------------------------------------------------------

@pytest.fixture()
def fat_rows(tmp_path: Path) -> Path:
    """A file whose individual rows are large, not merely numerous."""
    path = tmp_path / "fat.jsonl"
    blob = "x" * 200_000
    with path.open("w", encoding="utf-8") as out:
        for identity in range(20):
            out.write(json.dumps({"id": identity, "doc": blob}) + "\n")
    return path


def test_a_page_of_large_rows_stops_at_the_budget(fat_rows: Path) -> None:
    """The row count is a ceiling, not a promise."""
    with open_memory_connection() as conn:
        page = execute_page(
            conn, _select(fat_rows), offset=0, limit=50, max_bytes=500_000,
        )

    # Asked for fifty. Two rows fit the budget; the third would not.
    assert len(page.rows) < 50
    assert page.next_offset == len(page.rows)


def test_rows_are_never_clipped_to_fit(fat_rows: Path) -> None:
    """The whole point: a page carries fewer rows, never smaller ones."""
    with open_memory_connection() as conn:
        page = execute_page(
            conn, _select(fat_rows), offset=0, limit=50, max_bytes=10,
        )

    # A budget nothing can satisfy still returns one row, whole.
    assert len(page.rows) == 1
    assert len(page.rows[0]["doc"]) == 200_000, "the value was altered to fit"


def test_small_rows_still_fill_the_requested_page(jsonl_file: Path) -> None:
    """The budget must not cost an ordinary page its rows."""
    with open_memory_connection() as conn:
        page = execute_page(conn, _select(jsonl_file), offset=0, limit=50)
...
def test_a_short_page_does_not_skip_the_rows_it_did_not_send(
    fat_rows: Path,
) -> None:
    """next_offset counts rows returned, not the limit asked for.

    Reading it from the limit would step over every row the budget held back,
    and they would never be seen again -- silently, because with no total there
    is nothing to check the arithmetic against.
    """
    seen: list[int] = []
    with open_memory_connection() as conn:
        offset: int | None = 0
        while offset is not None:
            page = execute_page(
                conn,
                f"{_select(fat_rows)} ORDER BY id",
                offset=offset,
                limit=50,
                max_bytes=500_000,
            )
